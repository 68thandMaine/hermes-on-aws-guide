...
[Chapter 31](31-secrets-management.md) externalized credentials. The platform can now **deploy**, **scale**, and **authenticate**—but you still cannot answer:
...
- [ ] Relate Prometheus metrics to [Chapter 28](../part-iv-kubernetes/28-scaling.md) HPA behavior
...
- [Chapter 25](../part-iv-kubernetes/25-helm.md) — Helm installed
- [Chapter 28](../part-iv-kubernetes/28-scaling.md) — HPA + metrics-server (`kubectl top`)
- [Chapter 31](31-secrets-management.md) — secrets externalized (monitoring stack is separate)
...
kubectl get nodes
helm version
kubectl top nodes    # metrics-server from Ch 28
```
...
This chapter is **monitoring**—the foundation. [Chapter 33](33-logging.md) adds logs (and tracing concepts) for event reconstruction.
...
| **Logs** | Discrete event streams | Chapter 33 |
| **Traces** | Request path across services | Chapter 33 (intro) |
...
| **Introduced** | [Chapter 28](../part-iv-kubernetes/28-scaling.md) | This chapter |

```text
metrics-server  →  HPA decisions (Ch 28)
Prometheus      →  human + alert visibility (Ch 32)
```
...
**kube-prometheus-stack** (Helm chart) installs the above as one unit—same packaging pattern as [Chapter 25](../part-iv-kubernetes/25-helm.md).
...
- Persistence requires explicit PVC strategy ([Chapter 24](../part-iv-kubernetes/24-kubernetes-storage.md) `local-path`)
...
### Step 4 — Connect to Chapter 28 Scaling

Re-run the [Chapter 28](../part-iv-kubernetes/28-scaling.md) load lab while watching Grafana:
...
Example template: `infrastructure/kubernetes/ch32-servicemonitor-hermes-bridge.example.yaml`
...
4. Run Ch 28 load generator; watch HPA and Grafana concurrently
5. Apply `ch32-prometheusrule-hermes-lab.yaml`; trigger high CPU; check Alertmanager
...
- [Chapter 28: Scaling](../part-iv-kubernetes/28-scaling.md)
...
[Chapter 33: Logging](33-logging.md) — centralized logs and request-flow reconstruction (tracing concepts) so you can debug individual events, not only aggregate metrics.

---

[← Chapter 31: Secrets Management](31-secrets-management.md) | [Next: Chapter 33 — Logging →](33-logging.md)
