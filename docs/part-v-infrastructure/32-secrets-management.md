...
[Chapter 27](../part-iv-kubernetes/27-configuration-configmaps-secrets.md) introduced Kubernetes Secrets. [Chapter 31](31-github-actions.md) introduced a **privileged automated actor** that mutates infrastructure with AWS credentials in GitHub Secrets.
...
Hermes needs model provider keys, tool API credentials, and database passwords. Those cannot live in Git, Helm values, or base64 YAML forever. Automated CI ([Chapter 31](31-github-actions.md)) makes credential leakage more dangerous—not less. External secret management is how the platform stays secure while evolving through merges.
...
- [Chapter 27](../part-iv-kubernetes/27-configuration-configmaps-secrets.md) — ConfigMaps and in-cluster Secrets
- [Chapter 28](../part-iv-kubernetes/28-kubernetes-security.md) — RBAC (who can read Secrets)
- [Chapter 31](31-github-actions.md) — CI credentials in GitHub Secrets
...
| Kubernetes | `app-secret` from Ch 27 | Base64 ≠ encryption; etcd exposure |
...
Kubernetes Secrets ([Chapter 27](../part-iv-kubernetes/27-configuration-configmaps-secrets.md)):
...
This chapter uses **AWS Secrets Manager** on `us-west-2`.
...
CI credentials ([Chapter 31](31-github-actions.md)) remain in GitHub Secrets for now—[Chapter 32 hardening note](#step-8--harden-ci-credentials-preview): prefer **OIDC federation** over long-lived access keys when Terraform apply runs in production.
...
```bash
aws secretsmanager describe-secret --secret-id hermes/api-key --region us-west-2
```
...
Compare to [Chapter 27](../part-iv-kubernetes/27-configuration-configmaps-secrets.md) `app-secret`—same injection pattern, different **source of truth**.

### Step 8 — Harden CI Credentials (Preview)

[Chapter 31](31-github-actions.md) stores `AWS_ACCESS_KEY_ID` in GitHub Secrets. Production hardening:
...
| Problem | Cause | Fix |
|---------|-------|-----|
| ClusterSecretStore Invalid | Wrong AWS creds or region | Check `eso-aws-credentials`; region `us-west-2` |
...
**CI leakage** — Terraform logs or fork PRs exposing GitHub Secrets. OIDC + environment protection ([Chapter 31](31-github-actions.md)).
...
- [Chapter 27: Configuration](../part-iv-kubernetes/27-configuration-configmaps-secrets.md)
- [Chapter 31: GitHub Actions](31-github-actions.md)
...
[Chapter 33: Monitoring](33-monitoring.md) — Prometheus, Grafana, and metrics so the platform can explain itself under load and failure.

---

[← Chapter 31: GitHub Actions](31-github-actions.md) | [Next: Chapter 33 — Monitoring →](33-monitoring.md)
