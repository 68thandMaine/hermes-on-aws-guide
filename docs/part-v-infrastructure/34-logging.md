...
[Chapter 33](33-monitoring.md) added **state visibility**—CPU curves, replica counts, alerts. You can see *that* something is wrong. You still cannot reconstruct:
...
- [Chapter 33](33-monitoring.md) — Grafana + Prometheus running (`monitoring` namespace)
...
- CPU is high ([Chapter 33](33-monitoring.md))
- HPA scaled out ([Chapter 29](../part-iv-kubernetes/29-scaling.md))
- Pods restarted
...
[Chapter 33](33-monitoring.md) covered metrics. This chapter completes the triad.

### Monitoring vs Event Reconstruction

```text
Monitoring (Ch 33)     →  aggregate state over time
Logging (Ch 34)        →  discrete events with context
Tracing (Ch 34)        →  request lifecycle across services
```
...
Prometheus (metrics)  +  Loki (logs)  +  Tempo (traces, optional)
        ↓
Grafana (single pane — Ch 33 UI, Ch 34 datasources)
        ↓
...
Reuse Grafana from [Chapter 33](33-monitoring.md)—disable bundled Grafana in the logging chart:
...
Port-forward Ch 33 Grafana if needed:
...
Deploy  →  Secure  →  Scale  →  Observe state  →  Observe events
  Ch 30–31   Ch 32      Ch 29       Ch 33              Ch 34
```
...
2. Add Loki datasource to Ch 33 Grafana
3. Run `{namespace="default"}` — confirm nginx-demo or system logs appear
4. Apply `ch34-structured-log-demo-pod.yaml`
5. LogQL: `{app="hermes-log-demo"} | json | event="tool_call"`
6. Optional: install Tempo; review `ch34-otel-collector-lab.example.yaml`
...
- [Chapter 33: Monitoring](33-monitoring.md)
...
[Chapter 35: Running Hermes](../part-vi-ai/35-running-hermes.md) — assemble and run the Hermes agent on everything built so far.

---

[← Chapter 33: Monitoring](33-monitoring.md) | [Next: Chapter 35 — Running Hermes →](../part-vi-ai/35-running-hermes.md)
