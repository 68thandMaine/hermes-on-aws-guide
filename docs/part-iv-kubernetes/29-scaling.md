...
- [ ] Map scaling to the Deployment reconciliation loop from [Chapter 22](22-deployments.md)

---

## Prerequisites

- [Chapter 22: Deployments](22-deployments.md) — `nginx-deployment` (or reinstall via Helm `nginx-demo`)
- [Chapter 23: Services](23-services.md) — `nginx-service` reachable in-cluster
- [Chapter 28: Security](28-kubernetes-security.md) — if NetworkPolicy blocks load-gen Pods, allow or remove policy for this lab
...
Deployment controller creates/destroys Pods
       ↓
Service + Ingress routes to new Pods (Ch 23–24)
```
...
If NetworkPolicy from [Chapter 28](28-kubernetes-security.md) blocks traffic, label `load-gen` with `access=frontend` (as above) or remove the policy for this lab.
...
Custom metrics come in [Chapter 33: Monitoring](../part-v-infrastructure/33-monitoring.md).
...
[Chapter 30: Terraform](../part-v-infrastructure/30-terraform.md) — codify the AWS foundation you built manually in Part II.

Observability under load (Prometheus, Grafana, alerts) continues in [Chapter 33: Monitoring](../part-v-infrastructure/33-monitoring.md)—after IaC and CI/CD give you reproducible environments.

---

[← Chapter 28: Security](28-kubernetes-security.md) | [Next: Chapter 30 — Terraform →](../part-v-infrastructure/30-terraform.md)
