...
[Chapter 22](22-deployments.md) keeps three nginx Pods running. Each Pod gets its **own IP**. When the Deployment replaces a Pod during self-healing or rollout, that IP disappears.
...
```text
Chapter 21   Pod         →  runs the process
Chapter 22   Deployment  →  keeps N Pods running
Chapter 23   Service     →  stable way to reach those Pods
```
...
- [ ] Contrast `port-forward` to a Pod (Ch 21) vs routing through a Service
...
- [Chapter 22: Deployments](22-deployments.md) — `nginx-deployment` with `app: nginx` labels and Running Pods
...
When you deleted a Pod in Chapter 22, the Deployment created a replacement with a **new IP**. Anything that cached the old IP broke.
...
| **Ingress** | HTTP routing | [Chapter 24](24-ingress.md) — `laptop → cluster` |
...
| Method | Use |
|--------|-----|
| `kubectl port-forward pod/...` | Debug one Pod from laptop (Ch 21) |
...
External users will use **Ingress** (Chapter 24), not port-forward.
...
2. Apply `ch23-nginx-service.yaml`
...
- Chapter 21 port-forward ≠ in-cluster Service routing
- Next: [Ingress](24-ingress.md)—HTTP from outside the cluster to Services
...
- [Chapter 22: Deployments](22-deployments.md)
...
[Chapter 24: Ingress](24-ingress.md) — route `laptop → Hermes` (and later HTTPS) to Services instead of port-forward hacks.

---

[← Chapter 22: Deployments](22-deployments.md) | [Next: Chapter 24 — Ingress →](24-ingress.md)
