...
Chapter 25       →  packaged system (helm install)
```
...
- [ ] Install the book’s **nginx-demo** chart (Ch 21–23 packaged)
...
- Chapters [21](21-deployments.md)–[24](24-kubernetes-storage.md) understood (objects the chart contains)
...
PVCs from Chapter 24 (`app-data`) are unrelated—leave them unless you are cleaning up that lab.
...
|----------|-------------------|
| `deployment.yaml` | Ch 21 — `nginx-deployment` |
| `service.yaml` | Ch 22 — ClusterIP Service |
| `ingress.yaml` | Ch 23 — Traefik Ingress `nginx.local` |
...
### Step 4 — Test Ingress (Same as Chapter 23)
...
| `INSTALLATION FAILED` conflict | Old manual resources same names | Delete Ch 21–23 objects (prerequisites) |
| Ingress 404 | Host/DNS unchanged from Ch 23 | Check `/etc/hosts`, Traefik |
...
4. Why package Ch 21–23 into one chart instead of three `kubectl apply` files?
...
- Next platform concerns: **configuration and secrets** before Hermes lands ([Chapter 31](../part-v-infrastructure/31-secrets-management.md) for production secret stores)
...
[Chapter 26: Configuration (ConfigMaps & Secrets)](26-configuration-configmaps-secrets.md) — separate code from configuration and credentials before locking down access.

Production secret storage hardens further in [Chapter 31: Secrets Management](../part-v-infrastructure/31-secrets-management.md).

---

[← Chapter 24: Storage](24-kubernetes-storage.md) | [Next: Chapter 26 — Configuration →](26-configuration-configmaps-secrets.md)
