...
- Part V complete ([Chapters 29–33](../part-v-infrastructure/29-terraform.md))
- k3s + Traefik Ingress ([Chapter 13](../part-ii-aws/13-the-first-control-plane.md), [Chapter 23](../part-iv-kubernetes/23-ingress.md))
- PVC storage class `local-path` ([Chapter 24](../part-iv-kubernetes/24-kubernetes-storage.md))
- Helm ([Chapter 25](../part-iv-kubernetes/25-helm.md))
- Recommended: monitoring + logging ([Chapters 32–33](../part-v-infrastructure/32-monitoring.md))
...
All Hermes workloads live here—RBAC and NetworkPolicy can scope to `namespace: hermes` ([Chapter 27](../part-iv-kubernetes/27-kubernetes-security.md)).
...
- Scale independently when GPU/CPU inference arrives ([Chapter 36](36-model-serving.md))
...
Chart creates Traefik Ingress for `hermes.local` ([Chapter 23](../part-iv-kubernetes/23-ingress.md) pattern):
...
**Metrics** ([Chapter 32](../part-v-infrastructure/32-monitoring.md)):

- Grafana → Pod CPU/memory for `namespace="hermes"`
- kube-state-metrics → replica counts for API and workers

**Logs** ([Chapter 33](../part-v-infrastructure/33-logging.md)):
...
Wire API keys via [Chapter 31](../part-v-infrastructure/31-secrets-management.md) `ExternalSecret` into `hermes` namespace before production Hermes lands—same pattern as `app-secret`, scoped to `hermes/*` in Secrets Manager.
...
**Worker starvation** — API accepts traffic but queue depth grows; workers underscaled. Fix: scale workers; later HPA on queue depth ([Chapter 28](../part-iv-kubernetes/28-scaling.md), Ch 32 custom metrics).

**Model bottleneck** — Inference latency spikes; worker backlog grows. Fix: scale model tier or GPU ([Chapter 36](36-model-serving.md)).
...
- [Chapter 36: Model Serving](36-model-serving.md)
...
[Chapter 35: Vector Databases](35-vector-databases.md) — vector storage for retrieval and memory workloads that Hermes will use.

[Chapter 36: Model Serving](36-model-serving.md) replaces the model stub with llama.cpp.

---

[← Chapter 33: Logging & Tracing](../part-v-infrastructure/33-logging.md) | [Next: Chapter 35 — Vector Databases →](35-vector-databases.md)
