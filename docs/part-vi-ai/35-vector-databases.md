...
[Chapter 34](34-running-hermes.md) made Hermes **execute**—API, workers, model, Redis, PostgreSQL. The system is alive but still lacks **semantic memory across time**:
...
- [Chapter 34: Running Hermes](34-running-hermes.md) — `hermes-lab` stack running
- [Chapter 24](../part-iv-kubernetes/24-kubernetes-storage.md) — `local-path` PVCs
...
|-------|-----------|-------------|---------|
| **Structured state** | PostgreSQL ([Ch 34](34-running-hermes.md)) | Rows, schemas, transactions | User ID, session config, tool run records |
| **Ephemeral coordination** | Redis ([Ch 34](34-running-hermes.md)) | Queues, cache, pub/sub | Task queue depth, rate limits |
...
### Monitoring vs Logging vs Vectors (Ch 32–33 bridge)
...
```bash
chmod +x infrastructure/aws/cli/ch35-vector-retrieval-demo.sh
...
**Metrics** — Qdrant exposes `/metrics` for Prometheus ([Chapter 32](../part-v-infrastructure/32-monitoring.md)); watch memory usage as collection grows.
...
[Chapter 36: Model Serving](36-model-serving.md) — replace the model stub with llama.cpp inference; connect retrieval output to the model layer.

---

[← Chapter 34: Running Hermes](34-running-hermes.md) | [Next: Chapter 36 — Model Serving →](36-model-serving.md)
