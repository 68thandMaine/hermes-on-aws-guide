...
# Chapter 37: GPU Instances

> A system's intelligence is bounded by its slowest layer of computation.

---

[Chapter 36](36-model-serving.md) brought **local reasoning** via llama.cpp on CPU. That works for learning—it is not enough for interactive agent loops at scale.
...
- [Chapter 36](36-model-serving.md) — CPU `llama-server` working
...
|------------|--------|
| Execute (Ch 34) | ✓ |
| Remember meaning (Ch 35) | ✓ |
| Reason locally (Ch 36) | ✓ CPU-bound |
| **Real-time cognition** | ✗ |
...
```bash
chmod +x infrastructure/aws/cli/ch37-gpu-node-prep.sh
NODE_NAME=<gpu-node-name> ./infrastructure/aws/cli/ch37-gpu-node-prep.sh
```
...
CPU path remains: `llama-server` on control plane ([Chapter 36](36-model-serving.md)).
...
[Chapter 38: The Hermes Reasoning Loop](38-ai-agent-architecture.md) — task-driven loop; workers own execution; models propose.

---

[← Chapter 36: Model Serving](36-model-serving.md) | [Next: Chapter 38 — The Hermes Reasoning Loop →](38-ai-agent-architecture.md)
