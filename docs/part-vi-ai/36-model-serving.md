...
[Chapter 35](35-vector-databases.md) added **semantic memory** (Qdrant). [Chapter 34](34-running-hermes.md) deployed execution layers—but the model stub was not real inference.
...
| Chapter | Capability |
|---------|------------|
| Ch 34 | System **executes** |
| Ch 35 | System **remembers** (meaning) |
| Ch 36 | System **reasons** (local inference) |
...
- [Chapter 34](34-running-hermes.md) — `hermes-lab` running
...
# On EC2 node via SSH:
ls -lh /models/model.gguf || ./infrastructure/aws/cli/ch36-prepare-model-lab.sh
```
...
| CPU/GPU resource limits | `resources` + device plugin ([Ch 37](37-gpu-instances.md)) | Harder to reason about in ops chapters |
...
```bash
chmod +x infrastructure/aws/cli/ch36-prepare-model-lab.sh
./infrastructure/aws/cli/ch36-prepare-model-lab.sh
...
```bash
chmod +x infrastructure/aws/cli/ch36-verify-llama-inference.sh
...
### Step 7 — Connect to RAG (Ch 35 Bridge)
...
**Metrics** — watch Pod memory during inference ([Chapter 32](../part-v-infrastructure/32-monitoring.md)):

- `container_memory_working_set_bytes{namespace="hermes",pod=~"llama-server.*"}`

**Logs** — llama.cpp server logs load time and requests ([Chapter 33](../part-v-infrastructure/33-logging.md)):
...
4. Run `ch36-verify-llama-inference.sh`
...
**Memory exhaustion** — Concurrent requests exceed RAM. Fix: `-np 1` parallel slots; queue at worker layer; GPU path ([Chapter 37](37-gpu-instances.md)).
...
| No GPU (until Ch 37) | Small models only |
...
5. How does Ch 36 change the Hermes system type?
...
- [Chapter 34: Running Hermes](34-running-hermes.md)
- [Chapter 35: Vector Databases](35-vector-databases.md)
...
[Chapter 37: GPU Instances](37-gpu-instances.md) — when CPU inference is insufficient; GPU nodes and model serving at scale.

[Chapter 38: The Hermes Reasoning Loop](38-ai-agent-architecture.md) — platform-owned task loop; memory + inference + tools.

---

[← Chapter 35: Vector Databases](35-vector-databases.md) | [Next: Chapter 37 — GPU Instances →](37-gpu-instances.md)
