...
[Chapter 41](41-platform-governance.md) established **how to run Hermes responsibly**. **Chapter 42 is where the book becomes yours**: most AI guides end with "here's how agents work." This one ends with **how to build new capabilities without changing the platform**.
...
- [ ] Register tools in the registry without changing the reasoning loop ([Ch 38](../part-vi-ai/38-ai-agent-architecture.md))
- [ ] Create new `agent_role` specializations and assign tools via policy ([Ch 41](41-platform-governance.md))
- [ ] Wire credentials through ESO—not prompts ([Ch 31](../part-v-infrastructure/31-secrets-management.md))
...
- Hermes reasoning loop ([Chapter 38](../part-vi-ai/38-ai-agent-architecture.md))
- Distributed agents ([Chapter 39](39-distributed-cognitive-execution.md))
- Governance + tool policy ([Chapter 41](41-platform-governance.md))
- Observability ([Chapters 32–33](../part-v-infrastructure/32-monitoring.md))
...
Application      →  Coordinator + agent roles (Ch 39)
...
The Ch 38 loop is the platform kernel:
...
| `maxLength` | Resource governance ([Ch 41](41-platform-governance.md)) |
...
|----------------|-------|
| Authentication | `GITHUB_TOKEN` from `ExternalSecret` ([Ch 31](../part-v-infrastructure/31-secrets-management.md)) |
...
Registration makes a tool **available** to workers. It does **not** grant permission—that is policy ([Ch 41](41-platform-governance.md)).
...
| `observability` | Links to Ch 32–33 pipelines |
...
|-------|-------|--------------|
| `weather` ([Ch 39](39-distributed-cognitive-execution.md)) | NOAA | Read-only external |
...
Connect denied attempts to alerts ([Ch 32](../part-v-infrastructure/32-monitoring.md))—a spike may indicate prompt injection probes ([Ch 41](41-platform-governance.md)).
...
| Kubernetes | ConfigMap mounted; worker SA unchanged ([Ch 41 RBAC](https://github.com/crudnicky/agent-to-aws-guide/blob/main/infrastructure/kubernetes/ch41-rbac-hermes-worker.yaml)) |
...
noaa.forecast v2  ──► compare latency + quality (Ch 40 model upgrade pattern)
```
...
New agent_roles:
  finance (already in Ch 39 ski-trip example)
  tax
...
Add tools + schemas + policy lines. Optionally add Qdrant collections for tax document embeddings ([Ch 35](../part-vi-ai/35-vector-databases.md))—that is **data**, not architecture.
...
| Failure recoverable? | Task retry + durable state (Ch 38) |
...
4. **Observe every capability** — extensions inherit Ch 32–33
5. **Test before deployment** — checklist as merge gate
6. **Govern before enabling** — register ≠ permit ([Ch 41](41-platform-governance.md))
...
2. Create sandbox GitHub token in AWS Secrets Manager; add ESO `ExternalSecret` (pattern from [Ch 31](../part-v-infrastructure/31-secrets-management.md))
...
- **Chapter 42 is the ownership graduation**—Hermes becomes *your* platform
...
- **Extensions inherit** security (Ch 41), observability (Ch 32–33), and operations (Ch 40)
...
- [Chapter 38: The Hermes Reasoning Loop](../part-vi-ai/38-ai-agent-architecture.md)
- [Chapter 39: Distributed Cognitive Execution](39-distributed-cognitive-execution.md)
- [Chapter 41: Security, Governance, and Trust](41-platform-governance.md)
...
[Chapter 43: From Development to Production](43-from-development-to-production.md) — promote Hermes to production with evidence, not optimism. The payoff: **you could actually run this.**

---

[← Chapter 41: Security, Governance, and Trust](41-platform-governance.md) | [Next: Chapter 43 — Development to Production →](43-from-development-to-production.md)
