...
# Chapter 40: Operating Hermes in Production
...
Parts I–VI and [Chapters 39](39-distributed-cognitive-execution.md) built Hermes. **Chapter 40 begins a different job**: you are no longer primarily a builder—you are an **operator** of a living distributed AI platform.

```text
Chapters 1–39:  "Does it work?"
Chapter 40+:    "Will it still work tomorrow while we change it?"
```
...
- [ ] Set SLOs and connect them to [Chapter 32](../part-v-infrastructure/32-monitoring.md) alerts
...
- Hermes lab stack running ([Chapter 34](../part-vi-ai/34-running-hermes.md))
- Monitoring + logging ([Chapters 32–33](../part-v-infrastructure/32-monitoring.md))
- CI pipeline awareness ([Chapter 30](../part-v-infrastructure/30-github-actions.md))
...
Models are **large binaries** on `/models` ([Ch 36](../part-vi-ai/36-model-serving.md))—not git diffs.
...
Task schema ([`task-schema.example.sql`](https://github.com/crudnicky/agent-to-aws-guide/blob/main/infrastructure/hermes/task-schema.example.sql)) changes follow the same discipline—especially with distributed tasks ([Ch 39](39-distributed-cognitive-execution.md)).
...
1. `terraform apply` — VPC, node ([Ch 29](../part-v-infrastructure/29-terraform.md))
...
Failures from **saturation**, not only crashes. Watch ([Ch 32](../part-v-infrastructure/32-monitoring.md)):
...
Scale on **measurements**—[Chapter 43](43-from-development-to-production.md) brings the full stack into production with operational discipline.
...
Link runbooks from Alertmanager annotations ([Ch 32](../part-v-infrastructure/32-monitoring.md)).
...
- EBS + snapshots
- Model storage size
- Log/metric retention (Ch 32–33 bounds)

Optimize from **observed** usage—stop GPU nodes when idle ([Ch 37](../part-vi-ai/37-gpu-instances.md)).
...
3. **Every failure is recoverable** (durable tasks, Ch 38)
...
- **Chapter 40 is the operator shift**—Hermes is alive; you preserve correctness under change
...
[Chapter 41: Security, Governance, and Trust](41-platform-governance.md) — authorization, auditability, prompt-injection resistance, human approvals, and AI-specific controls. **The language model is never the security boundary.**

---

[← Chapter 39: Distributed Cognitive Execution](39-distributed-cognitive-execution.md) | [Next: Chapter 41 — Security & Governance →](41-platform-governance.md)
