...
# Chapter 43: From Development to Production
...
**Chapter 43 is the payoff.** No new infrastructure primitives. No new AI concepts. For the first time, step back and see the whole:
...
- k3s lab ([Chapter 13](../part-ii-aws/13-the-first-control-plane.md), [Chapter 34](../part-vi-ai/34-running-hermes.md))
- Terraform + GitHub Actions ([Chapters 29–30](../part-v-infrastructure/29-terraform.md))
- Operating procedures ([Chapter 40](40-operating-hermes-in-production.md))
- Governance ([Chapter 41](41-platform-governance.md))
- Extensions ([Chapter 42](42-extending-hermes.md))
...
**Notice something important:** Hermes itself—the reasoning loop, task model, worker mediation, tool gateway—has **barely changed** since [Chapter 38](../part-vi-ai/38-ai-agent-architecture.md). The **environment around it** matured. That is the sign of a platform, not a demo.
...
|-----------|-------------------|------------|----------------|
| Kubernetes | Single-node k3s | Multi-node or HA k3s | [Ch 13](../part-ii-aws/13-the-first-control-plane.md), [Ch 28](../part-iv-kubernetes/28-scaling.md) |
| Hermes API | 1–2 replicas | 3+ replicas, rolling deploy | [Ch 40](40-operating-hermes-in-production.md), `values-production-rollout.yaml` |
| PostgreSQL | Single Pod + PVC | Managed RDS or replicated | [Ch 11](../part-ii-aws/11-persistent-storage.md), [Ch 34](../part-vi-ai/34-running-hermes.md) |
| Redis | Single instance | HA Redis or managed | [Ch 34](../part-vi-ai/34-running-hermes.md) |
| Qdrant | local-path PVC | Replicated volume or managed | [Ch 35](../part-vi-ai/35-vector-databases.md) |
| Model server | CPU inference | CPU pool + optional GPU pool | [Ch 36–37](../part-vi-ai/36-model-serving.md) |
| Monitoring | Lab Grafana | Persistent dashboards + on-call alerts | [Ch 32](../part-v-infrastructure/32-monitoring.md) |
| Logging | Short retention | Centralized long-term Loki | [Ch 33](../part-v-infrastructure/33-logging.md) |
| Secrets | Lab ESO setup | Rotated SM secrets, scoped IAM | [Ch 31](../part-v-infrastructure/31-secrets-management.md) |
| Capabilities | Ski-trip tools | Your domain tools via registry | [Ch 42](42-extending-hermes.md) |
...
Every change should begin the same way ([Chapter 30](../part-v-infrastructure/30-github-actions.md)):
...
- Network: Terraform module ([Ch 29](../part-v-infrastructure/29-terraform.md))
- Workloads: `hermes-lab` Helm chart ([Ch 34](../part-vi-ai/34-running-hermes.md))
- Observability: monitoring/logging Helm releases ([Ch 32–33](../part-v-infrastructure/32-monitoring.md))
- Rollouts: `values-production-rollout.yaml` ([Ch 40](40-operating-hermes-in-production.md))
...
Development tolerated failure. Production **assumes** failure and continues anyway ([Chapter 40](40-operating-hermes-in-production.md)).
...
| Workers | Multiple replicas + Redis locks | [Ch 38](../part-vi-ai/38-ai-agent-architecture.md) |
| Ingress | Traefik on k3s; multi-node for prod | [Ch 23](../part-iv-kubernetes/23-ingress.md) |
| Databases | Backups + restore drills; managed HA when ready | [Ch 40](40-operating-hermes-in-production.md) |
...
Scale **independently**. Each subsystem has different saturation signals ([Chapter 28](../part-iv-kubernetes/28-scaling.md)):
...
Do not scale the entire platform because one component is hot. Measure, then scale the bottleneck—exactly as [Chapter 39](39-distributed-cognitive-execution.md) scales `agent_role=weather` workers independently from summary agents.
...
| Extensions | At least one capability added via Ch 42 checklist |
...
Production runs continuously. Optimize from **measurements** ([Chapter 16](../part-ii-aws/16-managing-platform-costs.md), [Chapter 40](40-operating-hermes-in-production.md)):

| Lever | Action |
|-------|--------|
| Workers | HPA on queue depth—not fixed over-provisioning |
| GPU | Schedule GPU nodes for inference bursts ([Ch 37](../part-vi-ai/37-gpu-instances.md)) |
| Logs | Retention bounds in Loki ([Ch 33](../part-v-infrastructure/33-logging.md)) |
| Vectors | Expire unused Qdrant collections ([Ch 35](../part-vi-ai/35-vector-databases.md)) |
...
```text
1. User submits request via Ingress (Ch 23)
2. API creates task in PostgreSQL (Ch 38)
3. Worker claims task via Redis lock
4. Qdrant retrieves semantic context (Ch 35)
5. llama-server completes inference (Ch 36)
6. Tool gateway authorizes and executes external call (Ch 41–42)
7. Step persisted to hermes_task_steps
8. Prometheus records latency (Ch 32)
9. Structured log line ships to Loki (Ch 33)
...
- New capabilities ([Chapter 42](42-extending-hermes.md))
- Model upgrades with parallel validation ([Chapter 40](40-operating-hermes-in-production.md))
- Security policy updates ([Chapter 41](41-platform-governance.md))
- Coordinator workflows ([Chapter 39](39-distributed-cognitive-execution.md))
...
Git
      │
GitHub Actions          (Ch 30)
      │
Terraform             (Ch 29)
      │
AWS VPC + EC2         (Ch 7–8)
      │
Kubernetes / k3s      (Ch 13–28)
      │
Ingress               (Ch 23)
      │
Hermes API            (Ch 34)
      │
Workers + reasoning loop (Ch 38)
      │
Redis │ PostgreSQL │ Qdrant
      │
Model Server          (Ch 36–37)
      │
Tools + capabilities  (Ch 41–42)
      │
Monitoring / Logging  (Ch 32–33)
      │
Response
```
...
- **Chapter 43 is the payoff**—the first time the whole system feels real
...
- [Chapter 40: Operating Hermes in Production](40-operating-hermes-in-production.md)
- [Chapter 30: GitHub Actions](../part-v-infrastructure/30-github-actions.md)
- [Chapter 29: Terraform](../part-v-infrastructure/29-terraform.md)
- [`production-readiness.example.yaml`](https://github.com/crudnicky/agent-to-aws-guide/blob/main/infrastructure/hermes/production-readiness.example.yaml)
...
Production operating model (Ch 43) ✓
...
[Chapter 44: The Platform You Built](44-the-platform-you-built.md) — step back from implementation. Revisit the journey from the first Linux process to the distributed cognitive system running in AWS. Understand what you built—and why each layer matters.

No new commands. No new YAML. **Understanding.**

---

[← Chapter 42: Extending Hermes](42-extending-hermes.md) | [Next: Chapter 44 — The Platform You Built →](44-the-platform-you-built.md)
