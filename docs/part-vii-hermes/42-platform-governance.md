...
# Chapter 42: Security, Governance, and Trust

> A system that can act must also know when not to act.

---

[Chapter 41](41-operating-hermes-in-production.md) made you an **operator**—rolling deploys, backups, SLOs, runbooks. **Chapter 42 graduates the book** from "how to run an AI platform" to **how to run one responsibly**.

```text
Chapter 41:  "Will it still work tomorrow?"
Chapter 42:  "Who is allowed to make it do that?"
```
...
- [ ] Reconstruct actions from `hermes_task_steps` + Loki/Tempo ([Ch 34](../part-v-infrastructure/34-logging.md))
- [ ] Enforce infrastructure limits (Kubernetes) and cognitive limits (Hermes config)
- [ ] Keep secrets out of prompts, logs, and vector memory ([Ch 32](../part-v-infrastructure/32-secrets-management.md))

---

## Prerequisites

- Hermes lab running ([Chapter 35](../part-vi-ai/35-running-hermes.md))
- RBAC + NetworkPolicy baseline ([Chapter 28](../part-iv-kubernetes/28-kubernetes-security.md))
- External secrets ([Chapter 32](../part-v-infrastructure/32-secrets-management.md))
- Distributed tasks ([Chapter 40](40-distributed-cognitive-execution.md))
- Production operations mindset ([Chapter 41](41-operating-hermes-in-production.md))
...
- decompose objectives into coordinated subtasks ([Ch 40](40-distributed-cognitive-execution.md))
- run inference locally ([Ch 37](../part-vi-ai/37-model-serving.md))
...
```text
          ...
          Authentication          ← Ingress / API key / future IdP
                   │
              Hermes API          ← Deployment + Service (Ch 35)
                   │
...
This unifies [Chapter 28](../part-iv-kubernetes/28-kubernetes-security.md) RBAC, [Chapter 32](../part-v-infrastructure/32-secrets-management.md) secrets, [Chapters 33–34](../part-v-infrastructure/33-monitoring.md) observability, [Chapter 40](40-distributed-cognitive-execution.md) agents, and [Chapter 41](41-operating-hermes-in-production.md) operations into one principle: **governance is architecture**.
...
- validated infrastructure (Terraform state, signed images from [Ch 31](../part-v-infrastructure/31-github-actions.md))
...
| CI deploy | GitHub Actions OIDC / IAM | Git commit → image tag → rollout ([Ch 41](41-operating-hermes-in-production.md)) |
| Scheduled job | CronJob SA | `owner_id` = `system:scheduler` |

Correlation fields from [Chapter 40](40-distributed-cognitive-execution.md):

```text
HTTP request
    → root_request_id (one user objective)
        → task_id per subtask
            → trace_id per step (Ch 34)
```
...
Workers do not execute arbitrary commands from model output. The Ch 39 loop already mediates tools—Chapter 42 makes that mediation **explicit and enforceable**:
...
**Logs** — structured JSON from workers ([Ch 34](../part-v-infrastructure/34-logging.md)):

```json
{"level":"info","event":"tool_executed","tool":"noaa.forecast","agent_role":"weather","trace_id":"…","authorization":"allowed"}
```

**Metrics** — denied tool attempts as a Prometheus counter ([Ch 33](../part-v-infrastructure/33-monitoring.md)):

```text
hermes_tool_denied_total{agent_role="summary", tool="postgres.delete"}
```

**AWS** — CloudTrail for Secrets Manager access ([Ch 32](../part-v-infrastructure/32-secrets-management.md)); Terraform state for infra changes ([Ch 30](../part-v-infrastructure/30-terraform.md)).
...
- recursive coordinator loops ([Ch 40](40-distributed-cognitive-execution.md))
- repeated API retries
- runaway vector searches ([Ch 36](../part-vi-ai/36-vector-databases.md))
- GPU inference while idle ([Ch 38](../part-vi-ai/38-gpu-instances.md))

Connect cost alerts to [Chapter 16](../part-ii-aws/16-managing-platform-costs.md) practices and Ch 41 SLOs.
...
From [Chapter 32](../part-v-infrastructure/32-secrets-management.md): workers obtain credentials **only when executing an authorized tool**. Redact secrets in `payload_json` before persist. Loki pipelines should strip known key patterns.
...
On k3s single-node labs, enforcement may be partial ([Ch 28](../part-iv-kubernetes/28-kubernetes-security.md) troubleshooting). The **policy-as-code** still documents intent for multi-node production.
...
6. **Every model version is identifiable** — log GGUF path / tag in infer steps ([Ch 37](../part-vi-ai/37-model-serving.md))
...
Audit log (Postgres step + Loki + trace)
   │
Monitoring alert if anomaly (Ch 33)
```
...
2. Apply `ch42-rbac-hermes-worker.yaml`; patch worker Deployment to `serviceAccountName: hermes-worker`
3. Apply `ch42-networkpolicy-hermes.yaml` (skip if lab CNI blocks all traffic—document result)
...
- **Chapter 42 is the responsibility graduation**—from operating to governing
...
- **Layered controls** reuse RBAC (Ch 28), secrets (Ch 32), observability (Ch 33–34), agents (Ch 40), and ops (Ch 41)
...
[Chapter 43: Extending Hermes](43-extending-hermes.md) — add capabilities through tools and contracts without changing the platform kernel.

---

[← Chapter 41: Operating Hermes in Production](41-operating-hermes-in-production.md) | [Next: Chapter 43 — Extending Hermes →](43-extending-hermes.md)
