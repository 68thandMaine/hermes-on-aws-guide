# Chapter 31 — create demo secret in AWS Secrets Manager (lab)
# Usage: AWS_PROFILE=hermes ./ch31-create-hermes-api-secret.sh
...
if aws secretsmanager describe-secret --secret-id "$SECRET_NAME" --region "$AWS_REGION" >/dev/null 2>&1; then
  ...
  aws secretsmanager create-secret \
    --name "$SECRET_NAME" \
    --description "Hermes API key — Chapter 31 lab (not production)" \
    --secret-string "$SECRET_VALUE" \
    --region "$AWS_REGION"
fi
