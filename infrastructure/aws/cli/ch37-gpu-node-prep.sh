#!/usr/bin/env bash
# Chapter 37 — GPU node prep checklist for k3s + Hermes (run on GPU EC2 + operator laptop)
set -euo pipefail
...
echo "   kubectl apply -f infrastructure/kubernetes/ch37-nvidia-device-plugin.yaml"
